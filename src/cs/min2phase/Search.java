...
final public class Search {
	...
	String Solve(CubieCube c) {
		...
		for (int i=0; i<6; i++) {
			...
			e10[i] = c.getURtoUL();
			e20[i] = c.getDRtoDL();
			prun[i] = Math.max(CoordCube.UDSliceTwistPrun[twist[i] * 495 + CoordCube.UDSliceConj[slice[i]][tsym[i]]],
	...
	boolean phase1(int twist, int tsym, int flip, int fsym, int slice, int maxl, int lm) {
		...
		for (int m=0; m<18; m++) {
			...
			flipx >>>= 3;
			if (CoordCube.UDSliceFlipPrun[flipx * 495 + CoordCube.UDSliceConj[slicex][fsymx]] >= maxl) {
				continue;
			}
